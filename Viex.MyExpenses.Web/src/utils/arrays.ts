...
export default {
    ...
    uniqueBy<T>(arr: T[], property: string): T[] {
        const propertyValues = arr.map(item => objects.dot(item, property));
        const uniquePropertyValues = new Set(propertyValues);
        const uniqueArray: T[] = [];

        uniquePropertyValues.forEach(value => {
            const firstFound = arr.find(item => objects.dot(item, property) == value);
            uniqueArray.push(firstFound);
        });
